...
#define lcolor mix(vec3(1.0,0.5,0.2),vec3(1.2,0.2,0.1),clamp(time-10.0,0.0,1.0))
#define lights 16.0
#define crates 32.0
#define barrels 8.0
#define alarm clamp(time-10.0,0.0,2.0)
...
vec3 color(vec3 p,vec3 norm, vec3 d)
{
    ...
    float S = 0.3;//pow(clamp(dot(reflect(norm,d),normalize(light-p)),0.0,1.0)*0.5+texture(iChannel0,uv).r*0.5,8.0);
    float f = 1.0-pow(srand(vec3(floor(p.x/lights))),4.0)*srand(vec3(time*8.0))-cos(time*8.0)*alarm*0.5;
...
void main()
{
    vec2 f = (-resolution.xy + 2.0*fragCoord.xy)/resolution.y;//2D Position
    vec3 p = vec3((time-10.0)*(2.0+clamp(time-10.0,0.0,4.0)),srand3(vec3(0.0,time*2.0,0.0)).xy*alarm*0.2-alarm*0.1);//3D Position
